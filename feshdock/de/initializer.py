...
from feshdock.de.swarm import Swarm
from feshdock.de.coordinates import CoordinatesFileReader, Coordinates
from feshdock.error.lightdock_errors import GSOCoordinatesError
from feshdock.de.searchspace.landscape import (
    LandscapePosition,
...
class LightdockFromFileInitializer(Initializer):
    ...
    def __init__(
        ...
        self.step_translation = step_translation #0.5
        self.step_rotation = step_rotation#0.5
    ...
    def generate_landscape_positions(self):
        """Generates a list of landscape positions that have been read from initial_population_file.
        """
        reader = CoordinatesFileReader(self.dimensions)
        coordinates = reader.get_coordinates_from_file(self.initial_population_file)

        number_of_glowworms=len(coordinates)
        positions = []

        for i, adapter in enumerate(self.adapters):
            positions.append([])
            for index in range(number_of_glowworms):
                receptor_index = self.random_number_generator.randint(
                    0, len(self.adapters[0].receptor_model) - 1
                )
                ligand_index = self.random_number_generator.randint(
                    0, len(self.adapters[0].ligand_model) - 1
                )
                pos=  DockingLandscapePosition(
                        self.objective_functions[i],
                        coordinates[index],
