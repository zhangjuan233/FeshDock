...
from feshdock.prep.simulation import (
    read_input_structure,my_read_input_structure
)
...
class DockingLandscapePosition(LandscapePosition):
    ...
    def qua2angle(self):
        norm = np.sqrt(self.rotation.w ** 2 + self.rotation.x ** 2 + self.rotation.y ** 2 + self.rotation.z ** 2)
        self.rotation.w /= norm
        self.rotation.x /= norm
        self.rotation.y /= norm
        self.rotation.z /= norm
        w = self.rotation.w
        x = self.rotation.x
        y = self.rotation.y
        z = self.rotation.z
        theta = 2*np.arccos(w)
        alpha = np.arccos(z / (np.sin(theta / 2))) #[-1,1]
        flag = y / (np.sin(alpha) * np.sin(theta / 2))
        if flag >= 0:
            beta = np.arccos(x / (np.sin(alpha) * np.sin(theta / 2)))
        else:
            beta = 2 * math.pi - np.arccos(x / (np.sin(alpha) * np.sin(theta / 2)))
        self.angles = [theta, alpha, beta]
    ...
    def distance2(self, other):
        ...
        ligand_pose has been already calculated in the update_luciferin stage of the algorithm.
        """
        rmsd2 = np.sum(
