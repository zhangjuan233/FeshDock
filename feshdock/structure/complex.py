...
from docking.constant import ParamterStore
...
class Complex(object):
    ...
    def move_to_origin(self):
        ...
        return translation
    def my_move_to_origin(self,vector):
        """Moves the structure to the origin of coordinates"""
        # translation = [-1 * c for c in self.center_of_coordinates()] # 计算质心  计算需要平移的向量（取反）
    ...
    def rec_fft1(self):
        num_fft=self.para.num_fft
        num_grid=self.para.num_grid
        ndata =(num_fft-num_grid)//2 # 根据实际情况设置
        FFTWin = np.zeros((num_fft**3), dtype=np.complex128)

        # 将数据存储到FFTWin数组中
        m = 0
        for i in range(num_grid):
            ic = num_fft * num_fft * (i + ndata)
            for j in range(num_grid):
                jc = ic + num_fft * (j + ndata)
                for k in range(num_grid):
                    FFTWin[jc+k+ndata] = complex(self.grid_r[m], self.grid_i[m])
                    m += 1

        # 进行3D FFT变换
        FFTWin =pyfftw.interfaces.numpy_fft.fftn(FFTWin)

        # FFTWin_3d = np.reshape(FFTWin, (num_fft, num_fft, num_fft))
        # FFTWin_3d_transformed = pyfftw.interfaces.numpy_fft.fftn(FFTWin_3d)
        # self.fft_result= FFTWin_3d_transformed

        # 将变换结果存储到_FFT_rec_r和_FFT_rec_i中
        nf3 = num_fft * num_fft * num_fft
        self.fft_r = np.real(FFTWin.flatten()[:nf3])
        self.fft_i = np.imag(FFTWin.flatten()[:nf3])
        # print('fft')
    ...
    def rpscace_r(self,ACE_ratio,rec_core,ligand_core):
        ...
        for l in range(self.num_atoms):
            ...
            ja=max(j2-search_range,0)
            ka=max(k2-search_range,0)
            ib=min(i2+search_range+1,ng1)#上界
            jb=min(j2+search_range+1,ng1)
            kb=min(k2+search_range+1,ng1)
            lc=ng1*l
        ...
        # 根据"cavity"区域中已经标记为开放空间的网格，通过填充相邻网格来扩展开放空间的范围  （应该就是没有原子填充的网格 ）
        for ink in range(2):
    ...
    def voxel_init(self):#计算当前原子与每个网格在三个维度上的差值的平方
        ...
        for i in range(self.num_atoms):
            for j in range(self.para.num_grid):
                self.para.xd.append(self.atom_coordinates[0][i][0]-self.grid_coord[j])#列表大小为原子数*网格数
    ...
    def ligand_init(self, num_grid):
        self.para.num_grid = num_grid
        self.para.num_fft=num_grid*2
        ng1=self.para.num_grid
        ng2=ng1**2
        Grid_coord = []
        ...
        # s1 = time.time()
        # a1=s1-s0
        self.grid_coord = search_length * (-0.5 + (0.5 + np.arange(ng1)) / ng1)
        # s2 = time.time()
        # a2=s2-s1
        ...
        # s1=time.time()
        # a1=s1-s0
        self.grid_r = np.zeros(ng2 * ng1)
        self.grid_i = np.zeros(ng2 * ng1)
        # s2=time.time()
        # a2=s2-s1
