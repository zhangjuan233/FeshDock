...
from feshdock.error.lightdock_errors import PDBParsingError, PDBParsingWarning
from feshdock.structure.atom import Atom, HetAtom
from feshdock.structure.residue import Residue
from feshdock.structure.chain import Chain
...
def parse_complex_from_file(
            ...
            if line_type == "MODEL":
                ...
                try:
                    ...
                    if verbose:
                        print(warning)
                    continue
                atomtype = atom.name + ' ' + atom.residue_name
                atomrtype_list.append(atomtype)
                # /////////
    ...
    return atoms, residues, chains,  atomrtype_list
...
def write_atom_line(atom, atom_coordinates, output):
    ...
    if atom.__class__.__name__ == "HetAtom":
        ...
        atom_type = "ATOM  "
    line = "%6s%5d %-4s%-1s%3s%2s%4d%1s   %8.2f%8.2f%8.2f%6.2f%6.2f%12s%s" % (
        atom_type,
        atom.number,
